...
func updateCustomer(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "PUT" {
		...
		if err != nil {
			...
			result, _ := stmt.Exec(CompanyName, ContactName, ContactTitle, Address, City, Region, PostalCode, Country, Phone, Fax, CustomerID)
...
func deleteCustomer(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		result, _ := stmt.Exec(customerID, companyName)
		affect, _ := result.RowsAffected()
		if affect == 0 {
			fmt.Fprintf(w, "delete failed")
		} else {
